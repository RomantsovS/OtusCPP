...
void map_reduce() {
    ...
    const auto mapper = [](auto num) { return num * num; };
    std::transform(input.begin(), input.end(), std::back_inserter(map_output), mapper);

    // Reduce
    const auto reducer = [](auto res, auto num) { return res + num; };
...
void map_shuffle_reduce() {
    std::vector<int> input{
        1, 6, 3, -7, 2, 15, -10, -1, 3, 2, -3, 7, -2, 15, 10,
    };

    // Map
    std::vector<int> map_output;
    const auto mapper = [](auto num) { return std::abs(num); };
    std::transform(input.begin(), input.end(), std::back_inserter(map_output), mapper);
...
std::string input(
    ...
    "method, which performs a summary operation (such as counting the number of students in each "
    "queue, yielding name frequencies). The MapReduce System (also called infrastructure or "
    "framework) orchestrates the processing by marshalling the distributed servers, running the "
...
std::string prepare(const std::string& in) {
    std::string lower;
    std::transform(in.begin(), in.end(), std::back_inserter(lower),
                   [](unsigned char c) { return std::tolower(c); });
    std::string alpha_lower;
    std::copy_if(lower.begin(), lower.end(), std::back_inserter(alpha_lower),
...
void almost_big_data() {
    ...
    const auto mapper = [](auto word) { return std::pair{word, 1}; };
    ...
    const auto last_word = std::accumulate(map_output.begin(), map_output.end(),
                                           std::pair<std::string, int>{"", 0}, reducer);
